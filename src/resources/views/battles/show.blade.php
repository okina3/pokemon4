            ...
            <div class="mb-3">
                ...
                <div class="mb-5">
                    ...
                    @foreach ($select_battle->oppSelects as $opp_Select)
                        <span
                            class="mr-2 p-2 font-semibold border border-gray-500 rounded">{{ $opp_Select->name }}</span>
                    @endforeach
                </div>
                {{-- 選択したバトルデータの自分の選出 --}}
                <div class="mb-5">
                    ...
                    @foreach ($select_battle->playerSelects as $player_select)
                        <span
                            class="mr-2 p-2 font-semibold border border-gray-500 rounded">{{ $player_select->name }}</span>
                    @endforeach
                </div>
                {{-- 選択したバトルデータの環境を表示 --}}
                <div class="mb-5">
