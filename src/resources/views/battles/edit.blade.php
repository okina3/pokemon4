        ...
        <div class="p-3">
            <form action="{{ route('update') }}" method="post">
                    ...
                    @for ($i = 1; $i <= 6; $i++)
                        <select class="w-40" name="opp_teams[{{ $i }}]">
                            ...
                            @foreach ($all_pokemon as $pokemon)
                                <option
                                    value="{{ $pokemon->id }}" {{ $opp_teams[$i - 1] == $pokemon->id ? 'selected' : '' }}>
                                    {{ $pokemon->name }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="mb-5">
                    ...
                    <textarea class="w-full rounded" name="comment" rows="7"
                              placeholder="ここにコメントを入力">{{ $select_battle->comment }}</textarea>
                    {{-- エラーメッセージ（コメント） --}}
